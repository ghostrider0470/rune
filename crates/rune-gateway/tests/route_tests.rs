...
async fn get_session_tree_surfaces_subagent_audit_metadata() {
    ...
    assert_eq!(child["latest_subagent_result"], serde_json::Value::Null);
}

#[tokio::test]
async fn get_session_tree_surfaces_latest_subagent_result_summary() {
    let session_repo = Arc::new(MemSessionRepo::new());
    let turn_repo = Arc::new(MemTurnRepo::new());
    let transcript_repo = Arc::new(MemTranscriptRepo::new());
    let model_provider: Arc<dyn ModelProvider> = Arc::new(FakeModelProvider);
    ...
    let root_id = Uuid::parse_str("aaaaaaaa-1111-1111-1111-aaaaaaaaaaaa").unwrap();
    let child_id = Uuid::parse_str("bbbbbbbb-2222-2222-2222-bbbbbbbbbbbb").unwrap();
    let now = chrono::Utc::now();
    ...
    transcript_repo
        .append(NewTranscriptItem {
            ...
            seq: 0,
            kind: "message".into(),
            payload: serde_json::json!({"content": "work in progress"}),
            created_at: now,
        })
        .await
        .unwrap();
    transcript_repo
        .append(NewTranscriptItem {
            ...
            payload: serde_json::json!({
                ...
                "summary": "Implemented priority routing follow-up",
                "artifacts": [
                    {
                        "name": "PR #999",
                        "kind": "pr",
                        "uri": "https://example.test/pr/999",
                        "content_type": null,
                        "description": null
                    }
                ]
    ...
    assert_eq!(child["id"], child_id.to_string());
    assert_eq!(
        child["latest_subagent_result"]["session_id"],
        child_id.to_string()
    );
    assert_eq!(
        child["latest_subagent_result"]["summary"],
        "Implemented priority routing follow-up"
    );
    assert_eq!(
        child["latest_subagent_result"]["artifacts"],
        serde_json::json!([
            {
                "name": "PR #999",
                "kind": "pr",
