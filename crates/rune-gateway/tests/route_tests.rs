...
#[tokio::test]
async fn ws_rpc_runtime_context_budget_reports_partition_usage_and_checkpoint() {
    use rune_gateway::logging::LogStore;
    use rune_gateway::tool_execution_repo::InMemoryToolExecutionRepo;
    use rune_gateway::ws_rpc::RpcDispatcher;
    ...
    let payload = dispatcher
        .dispatch(
            ...
            serde_json::json!({
                ...
                "checkpoint": {
                    "status": "planning",
                    "key_decisions": ["ship budget endpoint"],
                    "next_step": "wire runtime"
                }
            }),
        )
        .await
    ...
    assert_eq!(
        payload["checkpoint"]["partition_snapshot"]["Background"],
        150
    );
    assert!(
        payload["report"]["partitions"]
            .as_array()
            .unwrap()
            .iter()
            .any(|partition| partition["name"] == "Objective" && partition["token_count"] == 100)
    );
}

#[tokio::test]
async fn ws_rpc_runtime_context_budget_gc_compacts_and_returns_before_after_reports() {
    use rune_gateway::logging::LogStore;
    use rune_gateway::tool_execution_repo::InMemoryToolExecutionRepo;
    use rune_gateway::ws_rpc::RpcDispatcher;
    ...
    let payload = dispatcher
        .dispatch(
            "runtime.context_budget.gc",
            serde_json::json!({
                "total_capacity": 100,
                "items": [
                    { "partition": "history", "id": "h0", "token_count": 10 },
                    { "partition": "history", "id": "h1", "token_count": 10 },
                    { "partition": "history", "id": "h2", "token_count": 10 },
                    { "partition": "history", "id": "h3", "token_count": 10 },
                    { "partition": "history", "id": "h4", "token_count": 10 },
                    { "partition": "history", "id": "h5", "token_count": 10 },
                    { "partition": "background", "id": "bg", "token_count": 15, "summarized": true }
                ],
                "status": "busy",
                "key_decisions": ["compact hot history"],
                "next_step": "continue",
                "checkpoint_store_path": "/tmp/rune-heartbeat-gc-test.json"
            }),
        )
        .await
        .unwrap();

    assert_eq!(payload["before"]["total_used"], 75);
    assert_eq!(payload["after"]["total_used"], 75);
    assert_eq!(payload["checkpoint"]["status"], "busy");
    assert_eq!(
        payload["checkpoint_storage_key"],
        serde_json::Value::String("/tmp/rune-heartbeat-gc-test.json".into())
    );
    assert_eq!(payload["gc"]["NoAction"], serde_json::Value::Null);
...
async fn ws_rpc_runtime_context_budget_gc_persists_checkpoint_when_store_path_is_provided() {
    ...
    let checkpoint_path = checkpoint_dir.path().join("context-checkpoint.json");
    let payload = dispatcher
        .dispatch(
            "runtime.context_budget.gc",
            serde_json::json!({
                ...
                "items": [
                    ...
                    { "partition": "history", "id": "h4", "token_count": 10 },
                    { "partition": "history", "id": "h5", "token_count": 10 },
                    { "partition": "history", "id": "h6", "token_count": 10 },
                    { "partition": "history", "id": "h7", "token_count": 10 },
                    { "partition": "background", "id": "bg", "token_count": 15, "summarized": true }
                ],
                "status": "busy",
                "key_decisions": ["persist latest checkpoint"],
                "next_step": "continue",
                "checkpoint_store_path": checkpoint_path
            }),
        )
        .await
        .unwrap();

    assert_eq!(payload["before"]["total_used"], 95);
    assert_eq!(payload["after"]["total_used"], 50);
    assert_eq!(payload["checkpoint"]["status"], "busy");
    assert_eq!(
        payload["checkpoint_storage_key"],
...
#[tokio::test]
async fn ws_rpc_runtime_context_budget_rejects_invalid_items() {
    use rune_gateway::logging::LogStore;
    use rune_gateway::tool_execution_repo::InMemoryToolExecutionRepo;
    use rune_gateway::ws_rpc::{RpcDispatcher, RpcError};
...
#[tokio::test]
async fn get_session_status_prefers_explicit_resume_and_task_metadata_for_subagents() {
    let session_repo = Arc::new(MemSessionRepo::new());
    let turn_repo = Arc::new(MemTurnRepo::new());
    let transcript_repo = Arc::new(MemTranscriptRepo::new());
    let model_provider: Arc<dyn ModelProvider> = Arc::new(FakeModelProvider);
    ...
    let grandchild_id = Uuid::parse_str("cccccccc-cccc-cccc-cccc-cccccccccccc").unwrap();
    let now = chrono::Utc::now();
    session_repo
        .create(NewSession {
            id: child_id,
            ...
            status: "running".into(),
            workspace_root: None,
            channel_ref: Some("webchat:test".into()),
            ...
            metadata: serde_json::json!({
                ...
                "mode": "planner",
                "subagent_lifecycle": "waiting_on_child",
                "subagent_runtime_status": "attached",
                "subagent_runtime_attached": true,
                "subagent_status_updated_at": "2026-03-15T10:00:00Z",
                "subagent_last_note": "Waiting on delegated child result",
                "orchestration_status": "delegated",
                "delegation_roles": ["planner", "reviewer"],
                "delegation_depth": 2,
                "resume_hint": "resume by collecting the child result and continuing synthesis",
                "task_pick_reason": "child repo scan returned the best candidate diff",
                "task_pick_constraint": "preserve single-PR merge queue",
                "deferred_task_reason": "hold gateway rebuild until merge",
                "goal_key": "runtime:stalled-turn-recovery",
                "goal_owner_agent_id": "agent-parent",
                "goal_leased_at": "2026-03-15T09:58:00Z",
                "goal_lease_expires_at": "2999-01-01T00:00:00Z"
            }),
            created_at: now,
            updated_at: now,
            last_activity_at: now,
    ...
    transcript_repo
        .append(NewTranscriptItem {
            ...
            payload: serde_json::json!({"content": "Operator note: keep the PR queue clean"}),
            created_at: now,
        })
        .await
        .unwrap();
    transcript_repo
        .append(NewTranscriptItem {
            ...
            payload: serde_json::json!({"content": "Parent result summary that should appear in the audit card"}),
    ...
    transcript_repo
        .append(NewTranscriptItem {
            ...
            seq: 1,
            kind: "subagent_result".into(),
            payload: serde_json::json!({"content": "Grandchild shipped the focused diff for the stalled-turn detector"}),
    ...
    let response = app
        .oneshot(
            Request::get(format!("/sessions/{child_id}/status"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        json["next_task_reason"],
        "picked next slice: child repo scan returned the best candidate diff; winning constraint: preserve single-PR merge queue; deferred work: hold gateway rebuild until merge"
    );
    assert_eq!(
        json["resume_hint"],
        "resume by collecting the child result and continuing synthesis"
    );
    ...
    assert_eq!(
        ...
        "Operator note: keep the PR queue clean"
    );
    assert_eq!(
        json["audit"]["last_subagent_result_excerpt"],
        "Parent result summary that should appear in the audit card"
    );
    assert_eq!(
...
#[tokio::test]
async fn ws_rpc_session_send_rate_limits_bursty_webchat_browser_tokens_across_sessions() {
    let now = chrono::Utc::now();
    let session_id = Uuid::now_v7();
    let session_repo = Arc::new(MemSessionRepo::new());
    session_repo
...
#[tokio::test]
async fn ws_rpc_session_send_rate_limits_shared_webchat_browser_token_across_sessions() {
    let now = chrono::Utc::now();
    let session_repo = Arc::new(MemSessionRepo::new());
    let first_session_id = Uuid::now_v7();
    let second_session_id = Uuid::now_v7();

    for id in [first_session_id, second_session_id] {
        session_repo
            .create(NewSession {
                id,
                kind: "direct".into(),
...
async fn ws_rpc_doctor_run_matches_http_contract() {
    ...
    let app = build_test_app_with_config(config.clone(), Some("secret-token".into()));
    ...
    let http_body = body_json(http_response).await;
    ...
    let dispatcher = RpcDispatcher::new(state);
    let ws_body = dispatcher
        .dispatch("doctor.run", serde_json::json!({}))
        .await
        .unwrap();
...
#[tokio::test]
async fn ws_rpc_session_list_filters_by_browser_session_token() {
    let now = chrono::Utc::now();
    let session_repo = Arc::new(MemSessionRepo::new());
    let matching_id = Uuid::now_v7();
    let other_id = Uuid::now_v7();

    for (id, channel_ref) in [
        (matching_id, Some("webchat:browser-a".to_string())),
        (other_id, Some("webchat:browser-b".to_string())),
    ] {
        session_repo
            .create(NewSession {
                id,
                ...
                runtime_profile: None,
                policy_profile: None,
                metadata: serde_json::json!({ "browser_auth_mode": "session auth" }),
                created_at: now,
                updated_at: now,
                last_activity_at: now,
            })
    ...
    let items = body.as_array().unwrap();
    assert_eq!(items.len(), 1);
    assert_eq!(items[0]["id"], matching_id.to_string());
    assert_eq!(items[0]["channel"], "webchat:browser-a");
    assert_eq!(items[0]["metadata"]["browser_auth_mode"], "session auth");
}
...
#[tokio::test]
async fn protected_webchat_route_accepts_session_token_query_auth() {
    let (app, _) = build_test_app_parts(AppConfig::default(), Some("shared-secret".to_string()));

    let response = app
        .oneshot(
            Request::get("/webchat?session_token=browser-secret")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn protected_http_routes_reject_session_token_query_auth_for_non_webchat_routes() {
    let (app, _) = build_test_app_parts(AppConfig::default(), Some("shared-secret".to_string()));

    let response = app
        .oneshot(
            Request::get("/sessions?session_token=browser-secret")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn instance_health_reports_peer_identity_and_transport_metadata_from_health_payload() {
    let peer_listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let peer_addr = peer_listener.local_addr().unwrap();
    tokio::spawn(async move {
        let payload = serde_json::json!({
...
async fn doctor_run_reports_instance_topology_summary() {
    ...
    config.memory.semantic_search_enabled = true;

    let root = std::env::temp_dir().join(format!("rune-doctor-topology-{}", Uuid::now_v7()));
    config.paths.db_dir = root.join("db");
    config.paths.sessions_dir = root.join("sessions");
    config.paths.memory_dir = root.join("memory");
    config.paths.media_dir = root.join("media");
    ...
    let body = body_json(response).await;
    assert_eq!(body["topology"]["deployment"], "server");
...
#[tokio::test]
async fn get_session_tree_surfaces_subagent_audit_metadata() {
    let session_repo = Arc::new(MemSessionRepo::new());
    let turn_repo = Arc::new(MemTurnRepo::new());
    let transcript_repo = Arc::new(MemTranscriptRepo::new());
    let model_provider: Arc<dyn ModelProvider> = Arc::new(FakeModelProvider);
    ...
    session_repo
        .create(NewSession {
            ...
            metadata: serde_json::json!({
                ...
                "subagent_runtime_status": "not_attached",
                "subagent_runtime_attached": false,
                "subagent_status_updated_at": "2026-03-29T12:00:00Z",
                "subagent_last_note": "Review result routing before resuming",
            }),
            created_at: now,
            updated_at: now,
            last_activity_at: now,
    ...
    assert_eq!(child["audit"], Value::Null);
    assert_eq!(
...
#[tokio::test]
async fn get_session_tree_surfaces_goal_lease_metadata_for_subagents() {
    let session_repo = Arc::new(MemSessionRepo::new());
    let turn_repo = Arc::new(MemTurnRepo::new());
    let transcript_repo = Arc::new(MemTranscriptRepo::new());
    let model_provider: Arc<dyn ModelProvider> = Arc::new(FakeModelProvider);
    ...
    let root_id = Uuid::parse_str("cccccccc-cccc-cccc-cccc-cccccccccccc").unwrap();
    let child_id = Uuid::parse_str("dddddddd-dddd-dddd-dddd-dddddddddddd").unwrap();
    let now = chrono::Utc::now();
    ...
    session_repo
        .create(NewSession {
            ...
            metadata: serde_json::json!({
                ...
                "goal_owner_agent_id": "architect-2",
                "goal_lease_expires_at": lease_expires_at,
                "goal_leased_at": leased_at,
                "goal_lease_recovered_at": "2026-03-29T12:01:00Z",
                "goal_lease_recovered_from_agent_id": "architect-1"
            }),
            created_at: now,
            updated_at: now,
            last_activity_at: now,
...
#[tokio::test]
async fn get_session_tree_summarizes_descendant_team_rollup() {
    let session_repo = Arc::new(MemSessionRepo::new());
    let turn_repo = Arc::new(MemTurnRepo::new());
    let transcript_repo = Arc::new(MemTranscriptRepo::new());
    let model_provider: Arc<dyn ModelProvider> = Arc::new(FakeModelProvider);
    ...
    let root_id = Uuid::parse_str("d1111111-1111-1111-1111-111111111111").unwrap();
    let child_id = Uuid::parse_str("d2222222-2222-2222-2222-222222222222").unwrap();
    let grandchild_id = Uuid::parse_str("d3333333-3333-3333-3333-333333333333").unwrap();
    let now = chrono::Utc::now();
    ...
    for session in [
        NewSession {
            ...
            runtime_profile: None,
            policy_profile: None,
            metadata: serde_json::json!({"mode": "operator"}),
            created_at: now,
            updated_at: now,
            last_activity_at: now,
        },
        ...
        NewSession {
            ...
            policy_profile: None,
            metadata: serde_json::json!({
                "subagent_lifecycle": "blocked",
                "status_reason": "blocked on review handoff"
            }),
            created_at: now,
            updated_at: now,
            last_activity_at: now,
    ...
    transcript_repo
        .append(NewTranscriptItem {
            ...
            kind: "subagent_result".into(),
            payload: serde_json::json!({
                "summary": "Grandchild delivered the final delegated audit summary for the root team view."
            }),
            created_at: chrono::DateTime::parse_from_rfc3339("2026-03-30T09:15:00Z")
                .unwrap()
                .with_timezone(&chrono::Utc),
        })
        .await
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let json = body_json(response).await;
    assert_eq!(json["audit"]["descendant_sessions"], 2);
    assert_eq!(json["audit"]["descendant_subagent_results"], 1);
    assert_eq!(json["audit"]["active_descendants"], 1);
    assert_eq!(json["audit"]["waiting_descendants"], 1);
    assert_eq!(json["audit"]["blocked_descendants"], 1);
    assert_eq!(json["audit"]["lead_team_status"], "waiting_for_subagent");
    assert_eq!(
        json["audit"]["last_descendant_result_at"],
...
#[tokio::test]
async fn get_session_tree_exposes_descendant_audit_on_nested_subagents() {
    let session_repo = Arc::new(MemSessionRepo::new());
    let turn_repo = Arc::new(MemTurnRepo::new());
    let transcript_repo = Arc::new(MemTranscriptRepo::new());
    let model_provider: Arc<dyn ModelProvider> = Arc::new(FakeModelProvider);
    ...
    let root_id = Uuid::parse_str("f1111111-1111-1111-1111-111111111111").unwrap();
    let child_id = Uuid::parse_str("f2222222-2222-2222-2222-222222222222").unwrap();
    let grandchild_id = Uuid::parse_str("f3333333-3333-3333-3333-333333333333").unwrap();
    let now = chrono::Utc::now();
    ...
    transcript_repo
        .append(NewTranscriptItem {
            ...
            kind: "subagent_result".into(),
            payload: serde_json::json!({
                "summary": "Grandchild delivered the final delegated audit summary for the nested subagent view."
            }),
            created_at: chrono::DateTime::parse_from_rfc3339("2026-03-30T10:15:00Z")
                .unwrap()
                .with_timezone(&chrono::Utc),
        })
        .await
    ...
    let response = app
        .oneshot(
            Request::get(format!("/sessions/{child_id}/tree"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
