...
If you are wiring multiple Rune instances together, set `[instance]` in `config.toml` and check `http://127.0.0.1:8787/api/v1/instance/health` after startup to confirm identity, hashed/versioned capability manifest details (including configured models, active projects, and comms transport), and peer reachability. Then run `rune gateway instance-health` locally for the same summary over the CLI, and `rune gateway delegation-plan --strategy least_busy` (or `--strategy named --peer-id <peer>`) to inspect the sender/receiver contract before turning on cross-instance delegation.
