...
- [x] session status surface (`/status` + first-class `session_status` equivalent with model/usage/timing/flags)
  ...
  - 2026-03-26 tracker-truth follow-up: restart continuity remains intentionally narrower than full session resurrection. Current shipped behavior includes durable approval lifecycle metadata and startup restoration/indexing, but resumed-session user notifications, broader in-flight continuation guarantees, and restart-and-resume proof beyond those shipped slices remain explicit open scope under #245 / narrowed follow-on #284.
