...
- pre-tool and post-tool execution records are appended to the session transcript as `status_note` entries prefixed with `hook_pre_tool_call ` or `hook_post_tool_call ` followed by a JSON array of execution records
